                             ...
                             const SearchParams& params)
{
    ...
    while(!open_list.empty()){
        ...
        closed_list[int(current_node.self.x/distances.metersPerCell())][int( \
                    current_node.self.y/distances.metersPerCell())] = current_node.parent;

        // Goal check
        if (sqrt((current_node.self.x - goal.x)*(current_node.self.x - goal.x)+ \
            (current_node.self.y - goal.y)*(current_node.self.y - goal.y)) < 0.01)
        {         
            ...
            while (sqrt((current.x - start.x)*(current.x - start.x)+ \
                    (current.y - start.y)*(current.y - start.y)) > 0.01)
            {
                parent = closed_list[int(current.x/distances.metersPerCell())][int(current.y/distances.metersPerCell())];
        ...
        for(float x = current_node.self.x - distances.metersPerCell();x <= current_node.self.x + distances.metersPerCell(); x=x+distances.metersPerCell())
        {
            for(float y = current_node.self.y - distances.metersPerCell();y <= current_node.self.y + distances.metersPerCell(); y=y+distances.metersPerCell())
            {
                ...
                if (sqrt((current_node.self.x - x)*(current_node.self.x - x)+ \
                         (current_node.self.y - y)*(current_node.self.y - y) < 0.01)) continue;

                // check if the child is in grid
                if (!distances.isCellInGrid(int(x/distances.metersPerCell()), \
                    int(y/distances.metersPerCell()))) continue;

                // check if the child is an obstacle
                float obsDistance = distances(int(x/distances.metersPerCell()),int(y/distances.metersPerCell()));
                if (obsDistance <= params.minDistanceToObstacle) continue;

                // check if the child has been explored
                if (closed_list[int(x/distances.metersPerCell())][int(y/distances.metersPerCell())].utime != -1) continue;
